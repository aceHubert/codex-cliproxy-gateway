...
ZCode provider 选择读取（`zcode/config.ts` 的 `readSelection`）是双源的：优先读 3.12.3 的 `setting.json` 字段 `providerFamilyConnectionSelections[family].kind` 并在入口归一化为 legacy providerID（`individual-coding-plan` / `team-coding-plan` → `builtin:<family>-coding-plan`、`start-plan` → `builtin:<family>-start-plan`），使 `readRoute` 镜像查找、`isPlan()` 套餐判定、credentials watch 与协议转换零改动；条目缺失或形状非法时回退 legacy `modelProviderFamilySelectedKeys`——这不是兼容旧版客户端（本项目不做兼容），而是覆盖 3.12.3 自身磁盘只有 legacy 的三个窗口（迁移惰性持久化、api-key 模式被迁移跳过、team 连接未解析）；条目存在但 kind 未知时必须报错且消息含原值，禁止回退已冻结的 legacy 选择（`off-peak` 走 account provider mode 的独立 ticket 流程，不在本入口）。新增 kind 支持时只改这一处归一化，不得在下游引入套餐语义。
